...
class Structure
{
	...
	function nav_main()
	{
		...
		if (count($result->num_rows()) > 0)
		{
			...
			foreach ($result->result_array() as $entry_data)
			{
				...
				{
					$html .= "\n<li{$li_class}{$list_item_id}><a href=\"" . $item_uri . "\"{$a_class}>" . $title."</a></li>".'<li class="divider-vertical"></li>';
				}
			}
	...
	function nav_sub()
	{
		...
		foreach ($pages as $page)
		{
			...
			// THE BIG LIST OUTPUT
			$list_item = '<li'.$li_contents.'><a '.$li_contents.' href="'.$item_uri.'">'.$page_title.'</a></li>'."\n";
