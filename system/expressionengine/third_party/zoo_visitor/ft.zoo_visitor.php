...
// If this is a CP request, we have to run through field_id's
	...
	function zenbu_get_table_data($entry_ids, $field_ids, $channel_id, $output_upload_prefs, $settings)
	{
		...
		if (count($entry_ids) > 0) {
			...
			$sql          = 'SELECT entry_id, author_id, username, screen_name, email from exp_members mem LEFT JOIN exp_channel_titles ct ON (ct.author_id = mem.member_id) WHERE ct.entry_id IN (' . $in_entry_ids . ')';
